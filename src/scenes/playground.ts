...
const defaultInteractionLabels: Record<string, string> = {
  ...
  "toggle-light-disco": "Toggle Disco",
...
export class Playground implements PlaygroundActionContext {
  ...
  private discoEnabled = false;
  private merrySpeed = 0.9;
  private lowGravityUntil = 0;
  public addShadowCaster: ShadowCaster;
  private registerProjectileCallback?: ProjectileRegistrar;
  private updateAimCameraCallback?: AimCameraUpdater;
  private clearAimCameraCallback?: () => void;
  public discoLights: BABYLON.DirectionalLight[] = [];
  ...
  public createDiscoLight(scene: BABYLON.Scene, index: number, color = neonColors[(index + 1) % neonColors.length]): BABYLON.DirectionalLight {
    const light = new BABYLON.DirectionalLight(`disco_light_${index}`, BABYLON.Vector3.Zero(), scene);
  ...
  public registerProjectile(mesh: BABYLON.AbstractMesh, ownerId?: string) {
    this.registerProjectileCallback?.(mesh, ownerId);
  }
  ...
  public updateAimCamera(position: BABYLON.Vector3, target: BABYLON.Vector3) {
    this.updateAimCameraCallback?.(position, target);
  }

  public clearAimCamera() {
    this.clearAimCameraCallback?.();
  }
